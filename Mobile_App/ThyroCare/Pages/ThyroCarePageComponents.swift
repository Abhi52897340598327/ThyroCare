...
struct ThyroMedicalDisclaimer: View {
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "exclamationmark.triangle.fill")
                .font(.subheadline)
                .foregroundStyle(ThyroUI.amber)

            Text("ThyroCare uses an algorithm to estimate and log thyroid trends. Results may not be clinically accurate and are not a formal diagnosis. If something feels off or symptoms change, please see an endocrinologist.")
                .font(.caption)
                .foregroundStyle(.secondary)
        ...
        .padding(12)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(ThyroUI.amber.opacity(0.10))
        .clipShape(RoundedRectangle(cornerRadius: 8))
        .overlay {
            RoundedRectangle(cornerRadius: 8)
                .stroke(ThyroUI.amber.opacity(0.25), lineWidth: 1)
        }
    }
}
